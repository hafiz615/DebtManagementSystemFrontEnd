...
export default function DebtorPaymentPlan({
  ...
  const calculateStartDate = (prevItem, timePeriod, frequency) => {
    if (!prevItem) return new Date().toISOString().split("T")[0];
        ...
        >
          ...
          {intervalCommission?.length > 0 && (
            <Grid>
              ...
              <AmountTextField
                width={smallScreen ? "100%" : "8rem"}
                value={intervalCommission[index]?.commissionFee}
                readonly={true}
              />
            </Grid>
          )}
