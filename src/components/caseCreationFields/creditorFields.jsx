...
export default function CreditorFields({
  ...
  const [accountTitle, setAccountTitle] = useState(thisCaseData.creditor.accountTitle);
  ...
  const handleRemoveContact = (contactIndex) => {
    ...
    const updatedFinalCaseData = finalCaseData.map((creditor, index) => {
      if (index === caseIndex) {
        ...
        const updatedContacts = creditor.creditor.contacts.filter((_, cIndex) => cIndex !== contactIndex);
    ...
    <>
      ...
      >
        ...
        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
          <IconButton onClick={() => handleRemoveCase(caseIndex)} color="error">
            <Delete />
          </IconButton>
        </Grid>
        <Grid
        ...
        >
          <div>
            <Typography sx={{ fontFamily: "Nunito", fontWeight: "600" }} gutterBottom>
            ...
            <Grid container item xs={12}>
              <PaymentsTextFields
                ...
                onChange={(e) =>
                  handleCaseDataChange(caseIndex, "creditor.businessInformation.companyName", e.target.value)
                }
              />
              <PaymentsTextFields
                type="text"
                ...
                width={smallScreen ? "100%" : "97%"}
                value={thisCaseData.creditor.businessInformation.businessCategory}
                onChange={(e) =>
                  handleCaseDataChange(caseIndex, "creditor.businessInformation.businessCategory", e.target.value)
                }
              />
            ...
            <Typography sx={{ fontFamily: "Nunito", fontWeight: "600", marginTop: "1rem" }} gutterBottom>
              Creditor Details
            </Typography>
            <Grid container item xs={12} sx={{ display: "flex" }}>
              <PaymentsTextFields
                ...
                onChange={(e) =>
                  handleCaseDataChange(caseIndex, "creditor.basicInformation.fullName", e.target.value)
                }
              />
              <PaymentsTextFields
                type="text"
                ...
                onChange={(e) =>
                  handleCaseDataChange(caseIndex, "creditor.basicInformation.email", e.target.value)
                }
                error=""
              />
              <MuiPhoneTextField
                ...
                onChange={(e) =>
                  handleCaseDataChange(caseIndex, "creditor.basicInformation.phone", phoneNumberFormat(e))
                }
            ...
            <input
              ...
              onChange={(e) =>
                handleCaseDataChange(caseIndex, "creditor.notes", e.target.value)
              }
              style={{
                backgroundColor: Colors.BG_LIGHT_GRAY,
                height: "2.5rem",
      ...
      >
        ...
        <Grid item xs={12}>
          ...
          <PaymentsTextFields
            ...
            onChange={(e) =>
              handleCaseDataChange(caseIndex, "creditor.lastFundedDate", e.target.value)
            }
            max={today}
          />
        ...
        <Grid item xs={12} md={7} lg={8}>
          <Typography sx={{ fontFamily: "Nunito", fontWeight: "500", color: Colors.DARK_GRAY, marginRight: "1rem" }}>
            Historical Range
          </Typography>
          <Grid container spacing={2}>
            <Grid item xs={6}>
              <Typography sx={{ fontFamily: "Nunito", fontWeight: "600", marginRight: ".7rem" }}>
                Minimum*
              </Typography>
              <AmountTextField
                value={thisCaseData.creditor.historicalRange?.minimum}
                onChange={(e) =>
                  handleCaseDataChange(caseIndex, "creditor.historicalRange.minimum", parseInt(e.target.value))
                }
                onKeyDown={handleNumberInput}
              />
            </Grid>
            <Grid item xs={6}>
              <Typography sx={{ fontFamily: "Nunito", fontWeight: "600", marginRight: ".5rem" }}>
                Maximum*
              </Typography>
              <AmountTextField
                value={thisCaseData.creditor.historicalRange?.maximum}
                onChange={(e) =>
                  handleCaseDataChange(caseIndex, "creditor.historicalRange.maximum", parseInt(e.target.value))
                }
                onKeyDown={handleNumberInput}
              />
            </Grid>
